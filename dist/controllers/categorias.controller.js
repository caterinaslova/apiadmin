...
class CategoriaController {
    static createCategoria = async (req, res, next) => {
        try {
            const { nombreCategoria, descripcionCategoria, tipoCategoria, ordenEnMenu, validaActualmente, carpetaFotos } = req.body;
            const nombreCarpetaFotosVerificada = (0, slugify_1.default)(carpetaFotos.toLowerCase(), '-');
            const categoria = new categoria_modelo_1.default;
            categoria.codigoCategoria = (0, slugify_1.default)(nombreCategoria.toLocaleLowerCase(), '-');
    ...
    static getAllCategoria = async (req, res, next) => {
        let search = "";
        ...
        try {
            ...
            if (req.query.sort) {
                sort = schemas_1.SortCategoriaSchema.parse(req.query.sort);
            }
            let resultado = await categoria_modelo_1.default.find({ $or: [{ nombreCategoria: { $regex: search, $options: 'i' } }, { tipoCategoria: { $regex: search, $options: 'i' } }] });
            ...
            if (sort && sort === "ordenEnMenu")
                resultado.sort((a, b) => a.ordenEnMenu - b.ordenEnMenu);
            if (sort && sort === "-ordenEnMenu")
                resultado.sort((a, b) => b.ordenEnMenu - a.ordenEnMenu);
    ...
    static getCategoria = async (req, res, next) => {
        try {
    ...
    static updateCategoria = async (req, res, next) => {
        let imagen;
        let nombreImagen;
        try {
            ...
            const categoria = await categoria_modelo_1.default.findById(req.categoria.id);
            categoria.codigoCategoria = (0, slugify_1.default)(nombreCategoria.toLocaleLowerCase(), '-');
    ...
    static deleteCategoria = async (req, res, next) => {
        try {
            await categoria_modelo_1.default.deleteOne({ _id: req.categoria._id });
    ...
    static uploadImageCategoria = async (req, res, next) => {
        try {
            let imagen;
            let nombreImagen;
            if (req.files) {
                ...
                const tipoArchivo = Object.entries(req.files.imagenCategoria).filter((renglon) => {
                    if (renglon[0] === "mimetype")
                        return renglon[1];
                });
                if (tipoArchivo[0][1] !== "image/jpeg" && tipoArchivo[0][1] !== "image/png") {
                    throw new errors_1.BadRequestError('Debe ingresar un archivo con extensión jpg o png');
                }
                ...
                if (tamanoArchivo[0][1] > 1000000) {
                    throw new errors_1.BadRequestError('Debe ingresar un archivo de tamaño menor a 1mb');
                }
